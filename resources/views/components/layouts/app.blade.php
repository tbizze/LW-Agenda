    ...
    <x-mary-nav full-width class="">
        ...
        <x-slot:actions>
            ...
            <x-mary-dropdown label="{{ Auth::user()->name }}" class="btn-ghost btn-sm" right>
                ...
                <x-mary-menu-item title="{{ __('Profile') }}" link="{{ route('profile.show') }}" icon="o-user"
                    class=" w-40" />

                <!-- Authentication -->
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf
                    <x-mary-menu-item title="{{ __('Log Out') }}" link="{{ route('logout') }}"
                        @click.prevent="$root.submit();" icon="o-power" />
                </form>
            </x-mary-dropdown>
        </x-slot:actions>
    </x-mary-nav>
        ...
        <x-slot:sidebar drawer="main-drawer" collapsible class="bg-base-300/65">
            ...
            <x-mary-menu activate-by-route>
                ...
                <x-mary-menu-sub title="Eventos" icon="m-adjustments-horizontal">
                    <x-mary-menu-item title="Evento grupos" icon="m-arrow-small-right" :link="route('evento.grupos.index')"
                        wire:navigate.hover />
                    <x-mary-menu-item title="Evento áreas" icon="m-arrow-small-right" :link="route('evento.areas.index')"
                        wire:navigate.hover />
                    <x-mary-menu-item title="Evento locais" icon="m-arrow-small-right" :link="route('evento.locals.index')"
                        wire:navigate.hover />
                </x-mary-menu-sub>
                ...
                <x-mary-menu-sub title="Configurações" icon="o-cog-6-tooth">
                    ...
                    <x-mary-menu-item title="Permissões" icon="m-arrow-small-right" :link="route('admin.permissions.index')"
                        wire:navigate.hover />
                    <x-mary-menu-item title="Funções por usuário" icon="m-arrow-small-right" :link="route('admin.user-has-roles.index')"
                        wire:navigate.hover />
                </x-mary-menu-sub>
            </x-mary-menu>
        </x-slot:sidebar>
