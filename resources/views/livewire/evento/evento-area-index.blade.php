<div>
    ...
    <x-mary-card shadow class=" bg-white">
        <x-mary-table :headers="$headers" :rows="$evento_areas" striped @row-click="$wire.edit($event.detail.id)"
            with-pagination :sort-by="$sortBy">
            @scope('actions', $evento_area)
                <div class="flex gap-1">
                    @can('evento.areas.delete')
                        <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $evento_area->id }})" spinner
