<div>
    ...
    <x-mary-card shadow class=" bg-white">
        <x-mary-table :headers="$headers" :rows="$evento_grupos" striped @row-click="$wire.edit($event.detail.id)"
            with-pagination :sort-by="$sortBy">
            @scope('actions', $evento_grupo)
                <div class="flex gap-1">
                    @can('evento.grupos.delete')
                        <x-mary-button icon="o-trash" wire:click="confirmDelete({{ $evento_grupo->id }})" spinner
